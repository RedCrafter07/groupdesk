...
$('.menu-btn').on({
	mouseenter: () => {
		typeIn.type('Menu', 10, '.menu-btn span', false);
	},
	mouseleave: () => {
		typeIn.delete(10, '.menu-btn span', false);
	},
	click: () => {
		if (menu.on == false) {
			typeIn.type('Add new Window', 10, '.menu', false);
			menu.on = true;
		} else if (menu.on == true) {
			typeIn.delete(10, '.menu', false);
			menu.on = false;
		}
	}
});
